...
class $SchedulesTable extends Schedules with TableInfo<$SchedulesTable, Schedule> {
  ...
  Schedule map(Map<String, dynamic> data, {String? tablePrefix}) {
    ...
    return Schedule(
      ...
      startTime: attachedDatabase.typeMapping.read(DriftSqlType.int, data['${effectivePrefix}start_time'])!,
      endTime: attachedDatabase.typeMapping.read(DriftSqlType.int, data['${effectivePrefix}end_time'])!,
    );
  }
...
class Schedule extends DataClass implements Insertable<Schedule> {
  ...
  const Schedule({required this.id, required this.content, required this.date, required this.startTime, required this.endTime});
  ...
  Schedule copyWith({int? id, String? content, DateTime? date, int? startTime, int? endTime}) => Schedule(
        id: id ?? this.id,
  ...
  bool operator ==(Object other) =>
      ...
      (other is Schedule && other.id == this.id && other.content == this.content && other.date == this.date && other.startTime == this.startTime && other.endTime == this.endTime);
}
...
class $$SchedulesTableTableManager extends RootTableManager<_$LocalDatabase, $SchedulesTable, Schedule, $$SchedulesTableFilterComposer, $$SchedulesTableOrderingComposer,
    $$SchedulesTableProcessedTableManager, $$SchedulesTableInsertCompanionBuilder, $$SchedulesTableUpdateCompanionBuilder> {
  $$SchedulesTableTableManager(_$LocalDatabase db, $SchedulesTable table)
      : super(TableManagerState(
          db: db,
          table: table,
          filteringComposer: $$SchedulesTableFilterComposer(ComposerState(db, table)),
          orderingComposer: $$SchedulesTableOrderingComposer(ComposerState(db, table)),
          getChildManagerBuilder: (p) => $$SchedulesTableProcessedTableManager(p),
...
class $$SchedulesTableProcessedTableManager extends ProcessedTableManager<_$LocalDatabase, $SchedulesTable, Schedule, $$SchedulesTableFilterComposer, $$SchedulesTableOrderingComposer,
    $$SchedulesTableProcessedTableManager, $$SchedulesTableInsertCompanionBuilder, $$SchedulesTableUpdateCompanionBuilder> {
  $$SchedulesTableProcessedTableManager(super.$state);
}

class $$SchedulesTableFilterComposer extends FilterComposer<_$LocalDatabase, $SchedulesTable> {
  ...
  ColumnFilters<DateTime> get date => $state.composableBuilder(column: $state.table.date, builder: (column, joinBuilders) => ColumnFilters(column, joinBuilders: joinBuilders));

  ColumnFilters<int> get startTime => $state.composableBuilder(column: $state.table.startTime, builder: (column, joinBuilders) => ColumnFilters(column, joinBuilders: joinBuilders));

  ColumnFilters<int> get endTime => $state.composableBuilder(column: $state.table.endTime, builder: (column, joinBuilders) => ColumnFilters(column, joinBuilders: joinBuilders));
}

class $$SchedulesTableOrderingComposer extends OrderingComposer<_$LocalDatabase, $SchedulesTable> {
  ...
  ColumnOrderings<DateTime> get date => $state.composableBuilder(column: $state.table.date, builder: (column, joinBuilders) => ColumnOrderings(column, joinBuilders: joinBuilders));

  ColumnOrderings<int> get startTime => $state.composableBuilder(column: $state.table.startTime, builder: (column, joinBuilders) => ColumnOrderings(column, joinBuilders: joinBuilders));

  ColumnOrderings<int> get endTime => $state.composableBuilder(column: $state.table.endTime, builder: (column, joinBuilders) => ColumnOrderings(column, joinBuilders: joinBuilders));
}
